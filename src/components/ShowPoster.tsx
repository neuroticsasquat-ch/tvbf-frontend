 ...
 * this ticket is about placement — turning that into two links to one show
 * would double the tab stops on a twelve-card grid and announce every show
 * twice. So `children` is rendered inside the link, and the overlay layer
 * mirrors the image's aspect ratio to stay over the poster rather than over the
 * caption. A list row's text sits *beside* the poster and passes no caption, so
 * its link holds the image alone and `linkLabel` names it — the seam NEU-1190
 * §1 will use for one tabbable link per row; it is not that fix.
 */
export function ShowPoster({
  ...
  src: string | null;
  /** Accessible name for the poster's link, used when the link holds the image
   * alone. A caption names the link itself, so it is not applied over one. */
  linkLabel: string;
  size: ShowPosterSize;
  ...
  return (
    ...
    >
      <Link to={to} aria-label={children ? undefined : linkLabel} className="group relative block">
         ...
         * Their `top-1` is the image's top because the caption is below. */}
